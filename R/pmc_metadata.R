...
pmc_metadata <-function(doc){
   ...
   # cat( as.character(xml_find_all(doc, "//front//journal-meta")))
   pmcid <- xml_text(xml_find_first(doc,
             "//front//article-id[@pub-id-type='pmcid']"))
   if(!is.na(pmcid))   z[["PMCID"]] <- paste0("PMC", pmcid)
   t1 <- xml_text(xml_find_first(doc, "//front//article-title"), trim=TRUE)
   if(!is.na(t1)){
       z[["Title"]] <- t1
      a1 <- xml_text(xml_find_all(doc,
              "//front//contrib[not(@contrib-type='editor')]/name/given-names"))
      a2 <- xml_text(xml_find_all(doc,
