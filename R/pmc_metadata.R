...
pmc_metadata <-function(doc ){
   ...
   if(!is.na(p1)){
      ...
      p1 <- xml_text( xml_find_first(doc, "//front//article-meta/elocation-id"))

   }
   z[["Pages"]]  <- p1
   # More PUB Dates  - tags always sorted day, month, year?
   epub <- xml_text( xml_find_all(doc, "//front//pub-date[@pub-type='epub']/*") )
   if(length(epub) > 0) z[["Published online"]] <- paste(rev(epub), collapse="-")
   rec <- xml_text( xml_find_all(doc, "//front//history/date[@date-type='received']/*") )
   ...
   # Publisher?
   x <- xml_text( xml_find_first(doc,  "//front//journal-meta//publisher-name"))
